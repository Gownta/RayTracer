...
Intersection Algebraic::intersect(const Point3D & _origin, const Vector3D & ray) const {
  Intersection candidates[4];
  int k = intersections(_origin, ray, CLOSEST, candidates);
  ...
  /*double closest = INFINITY;
  for (int i = 0; i < k; ++i) closest = min(closest, candidates[i].distance);

  assert(closest > 0);

  for (int i = 0; i < k; ++i) if (candidates[i].distance == closest) return candidates[i];
  return Intersection();*/
...
Intersection Mesh::intersect(const Point3D & origin, const Vector3D & uray) const {
  ...
  int k = intersections(origin, uray, CLOSEST, candidates);
